...
function salvarCliente(event) {
    ...
    const valor = document.getElementById('valor').value ? parseFloat(document.getElementById('valor').value) : 0;
    ...
    const cliente = {
        ...
        pago,
        observacoes,
        dataRegistro: clienteEmEdicao ? clienteEmEdicao.dataRegistro : new Date().toISOString()
    };
...
function gerarRelatorioPDF() {
    ...
    const valorTotal = clientes.reduce((total, cliente) => total + (cliente.valor || 0), 0);
    const valorPendente = clientes.filter(c => !c.pago).reduce((total, cliente) => total + (cliente.valor || 0), 0);
...
function gerarRelatorioCSV() {
    ...
    clientesOrdenados.forEach(cliente => {
        ...
        const valor = cliente.valor ? cliente.valor.toString().replace('.', ',') : '0';
    ...
    link.setAttribute('download', 'clientes-dayane.csv');
...
function restaurarDados() {
    ...
    reader.onload = function(e) {
        try {
            ...
            // Restaurar botões padrão do modal
            const btnConfirm = document.getElementById('btnConfirm');
            ...
            btnCancel.onclick = () => fecharModal('confirmModal');
...
function formatarMoeda(valor) {
    return valor ? `R$ ${valor.toFixed(2).replace('.', ',')}` : 'R$ 0,00';
}
